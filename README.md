...
We started by reverse-engineering `elite.bin`, Christian Pinder's Agon port of *Elite: The New Kind*. It loads at `&040000`. Inside we found:

- a table of 29 ship names at offset 20908
- a table of ship headers at offset 61151, each 19 bytes, pointing at three sub-arrays
- the Cobra Mk III at offset 39548: **28 / 38 / 13** — an unmistakable signature

The edge records decoded cleanly:

```
[face1: 1 byte][face2: 1 byte][vertex A offset: 3 bytes][vertex B offset: 3 bytes]
```

where the vertex offset divided by 9 gives the vertex index (vertex records are 9 bytes). The vertex *coordinate packing*, however, defeated us. We tried eight field layouts — 24-bit signed, sign-in-bit-0, 16-bit at four alignments, 8-bit at seven alignments — and rendered every candidate. None produced a ship.

So we took the coordinates from the published blueprint instead. **This turned into the best validation we could have asked for: all 38 edges from the published source match, in the same order, what we had decoded from the binary.** Two independent sources agreeing on the topology means the geometry in this repo is right.

The lesson generalises: when reverse-engineering fights back, find a second source and use the agreement as your test.
